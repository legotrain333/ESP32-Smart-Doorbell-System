...
// Sleep configuration
#define SLEEP_DURATION_SEC 10  // Sleep for 10 seconds between checks
#define WAKEUP_DURATION_MS 3000 // Stay awake for 3 seconds after activity
...
// Function to enter light sleep (keeps WiFi and sensors active)
void enterLightSleep() {
  Serial.println("Entering light sleep mode...");
  Serial.flush(); // Make sure all serial data is sent
  
  // Configure wake-up sources for Arduino ESP32
  ...
  // Enter light sleep (keeps WiFi and some peripherals active)
  esp_light_sleep_start();
  
  // After wake-up, reinitialize if needed
...
void loop() {
  // Check if we should go to sleep (only if LED is OFF and not waiting for response)
  if (isAwake && !ledOn && !waitingForResponse && (millis() - lastActivityTime > WAKEUP_DURATION_MS)) {
    Serial.println("Going to sleep...");
    enterLightSleep();
  }

  // Timeout for waiting response (5 seconds)
  if (waitingForResponse && (millis() - lastButtonPress > 5000)) {
    Serial.println("Timeout waiting for response. Resetting...");
...
void onDataRecv(const esp_now_recv_info_t *recv_info, const uint8_t *incomingData, int len) {
  lastActivityTime = millis(); // Update activity time on message reception
